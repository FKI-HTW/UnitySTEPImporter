...
namespace ventus {
    ...
    const Standard_Boolean OCCAdapter::loadStepFile() {
        ...
        if (IFSelect_ReturnStatus::IFSelect_RetDone == loadStatus) {
            ...
            return reader.Transfer(loadedDocument) && prepareTriangulation();
        }
    ...
    void OCCAdapter::findProductsAndParts() {
        ...
        ////assembly->Dump(s, true);
        //TDF_IDFilter aFilter(false);
        //TDF_AttributeIndexedMap aMap = TDF_AttributeIndexedMap();
        //assembly->ExtendedDump(s, aFilter, aMap);
    ...
    Quantity_ColorRGBA OCCAdapter::findColor(const TDF_Label& label) {
        Quantity_Color colorRgb;
        Quantity_ColorRGBA colorRgba;

        Handle(XCAFDoc_ColorTool) colorHandler = new XCAFDoc_ColorTool();
        colorHandler->Set(label);

        if (colorHandler->IsSet(label, XCAFDoc_ColorType::XCAFDoc_ColorSurf)) 
            colorHandler->GetColor(label, XCAFDoc_ColorType::XCAFDoc_ColorSurf, colorRgb);
        else if (colorHandler->IsSet(label, XCAFDoc_ColorType::XCAFDoc_ColorGen))
            colorHandler->GetColor(label, XCAFDoc_ColorType::XCAFDoc_ColorGen, colorRgb);
        else if (colorHandler->IsSet(label, XCAFDoc_ColorType::XCAFDoc_ColorCurv)) 
            colorHandler->GetColor(label, XCAFDoc_ColorType::XCAFDoc_ColorCurv, colorRgb);
    ...
    void OCCAdapter::insertDataToModuleAssamblyMap(int id, const TDF_Label& label, const Handle(XCAFDoc_ShapeTool)& assembly, TopoDS_Shape& shape) {
        ...
        modelBaseData.transformation = findTransformation(shapeTool, label);
        modelBaseData.boundingBox = findBoundingBox(shape);
        modelBaseData.label = label;
        if (!shape.IsNull())
            modelBaseData.shape = shape;
        modelBaseData.color = findColor(label);
